...
int main()
{
    ...
    while(getline(ifile,str))
    {
        ...
        BYTE cipher[9]= {188, 138, 226, 123, 17, 150, 78, 176, '\0'};

        flag = 'N';
        int ans = 1;

        set_the_key(sw, key, r);
        des(plain, cipher_temp, r, flag);

        for(i=0; i<8; i++)
        {
            if(int(cipher[i]) != int(cipher_temp[i]))
            {
                ans = 0;
                break;
            }
